...
def main():
    ...
    prompt_file_path = os.path.join(project_root, "data", "prompts", "verbal_question_prompt.txt")
    input_csv_path = os.path.join(project_root, "data", "datasets", "verbal_questions_pool.csv")
    output_csv_path = os.path.join(project_root, "data", "generated_data", "verbal_answers.csv")

    # Define the header for the output CSV file
    output_header = ["id", "response", "model", "time_taken_seconds"] # Added "time_taken_seconds"
            ...
            with open(input_csv_path, "r", newline='', encoding="utf-8") as infile:
                ...
                for i, row in enumerate(csv_reader):
                    ...
                    try:
                        ...
                        print(f"Skipping row {current_row_index + 1} in {input_csv_path} due to missing column: {e}. Expected 'id' and 'problem'.")
                        continue

                    print(f"\nProcessing Question id: {question_id} (Row {current_row_index + 1})...")
                    ...
                    if "Error:" in response_text and time_taken == 0: # Indicates an error from query_ollama
                        print(f"Failed to get response for id {question_id}: {response_text}")
